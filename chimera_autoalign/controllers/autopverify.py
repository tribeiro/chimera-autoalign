...
class AutoPVerify(ChimeraObject, IPointVerify):
    ...
    """
    __config__ = {'overscan_config' : None,
    ...
    def _takeImage(self):
        ...
        self.imageRequest["exptime"] =self["exptime"] or 10
        ...
        self.imageRequest["filename"] = os.path.basename(ImageUtil.makeFilename("pointverify-$DATE"))

        frames = cam.expose(self.imageRequest)
        cam["fast_mode"] = old_mode
    ...
    def pointVerify(self):
        ...
        if self.ntrials == 0:
            ...
            # ra_img_center = tel.getRa()
            # dec_img_center = tel.getDec()
        ...
        logstr = "%s ra_tel = %f dec_tel = %f ra_img = %f dec_img = %f delta_ra = %s delta_dec = %s" % (
        image["DATE-OBS"], ra_img_center, dec_img_center, ra_wcs_center, dec_wcs_center, Coord.fromD(delta_ra).AS, Coord.fromD(delta_dec).AS)
